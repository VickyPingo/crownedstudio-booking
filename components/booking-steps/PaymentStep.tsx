...
function getPricingOptionPrice(option: ServicePricingOption, peopleCount: number): number {
  switch (peopleCount) {
    case 1: return option.price1
    case 2: return option.price2 > 0 ? option.price2 : option.price1
    case 3: return option.price3 > 0 ? option.price3 : option.price1
    default: return option.price1
  }
}
...
function getServicePrice(service: ServiceWithUpsells, peopleCount: number, pricingOption: ServicePricingOption | null | undefined): number {
  if (pricingOption) return getPricingOptionPrice(pricingOption, peopleCount)
  return getPriceForPeopleCount(service, peopleCount)
}

function calculateUpsellsTotal(selectedUpsellsByPerson: PerPersonUpsells, peopleCount: number, upsells: Upsell[]): number {
  let total = 0
...
function calculateTotalDuration(baseDurationMinutes: number, selectedUpsellsByPerson: PerPersonUpsells, peopleCount: number, upsells: Upsell[]): number {
  const upsellMap = new Map(upsells.map((u) => [u.id, u]))
  ...
  for (let person = 1; person <= peopleCount; person++) {
    ...
    for (const upsellId of personUpsells) {
      const upsell = upsellMap.get(upsellId)
      if (upsell && upsell.duration_added_minutes > maxAddonDuration) {
...
function getPerPersonUpsellSummary(selectedUpsellsByPerson: PerPersonUpsells, peopleCount: number, upsells: Upsell[]): PersonUpsellSummary[] {
  const upsellMap = new Map(upsells.map((u) => [u.id, u]))
  ...
  for (let person = 1; person <= peopleCount; person++) {
    ...
    for (const upsellId of personUpsellIds) {
      const upsell = upsellMap.get(upsellId)
      if (upsell) { personUpsells.push(upsell); total += upsell.price }
    }
    if (personUpsells.length > 0) summaries.push({ person, upsells: personUpsells, total })
  }
  return summaries
}
...
function isWeekendOrHoliday(dateString: string, publicHolidayDates: string[]): boolean {
  ...
  return dayOfWeek === 0 || dayOfWeek === 6 || publicHolidayDates.includes(dateString)
}
...
export function PaymentStep({ service, formData, businessHours, publicHolidayDates }: PaymentStepProps) {
  ...
  // ── Gift voucher state ───────────────────────────────────────────────────
  const [appliedGiftVoucher, setAppliedGiftVoucher] = useState<{
    code: string
    serviceName: string
    peopleCount: number
    amountPaid: number
  ...
  const upsellsTotal = calculateUpsellsTotal(formData.selectedUpsellsByPerson, formData.peopleCount, service.upsells)
  const totalDuration = calculateTotalDuration(service.duration_minutes, formData.selectedUpsellsByPerson, formData.peopleCount, service.upsells)
  const perPersonSummaries = getPerPersonUpsellSummary(formData.selectedUpsellsByPerson, formData.peopleCount, service.upsells)

  const checkRepeatCustomer = useCallback(async () => {
    const email = formData.clientEmail?.trim()
    const phone = formData.clientPhone?.trim()
    if (!email && !phone) { setIsRepeatCustomer(false); setRepeatCheckDone(true); return }

    let query = supabase.from('bookings').select('id, customer_id, customers!inner(email, phone)').eq('status', 'confirmed').limit(1)
    if (email && phone) {
    ...
    if (error) { setIsRepeatCustomer(false); setRepeatCheckDone(true); return }
    setIsRepeatCustomer(Array.isArray(confirmedBookings) && confirmedBookings.length > 0)
    setRepeatCheckDone(true)
  }, [formData.clientEmail, formData.clientPhone])

  useEffect(() => { checkRepeatCustomer() }, [checkRepeatCustomer])

  const isAfterHours = formData.selectedTime ? isAfterHoursSlot(formData.selectedTime, service.slug, businessHours) : false
  ...
  const weekendSurcharge = isWeekendOrHoliday(formData.selectedDate, publicHolidayDates) && weekendSurchargePP > 0
    ? weekendSurchargePP * formData.peopleCount : 0
  const subtotal = servicePrice + upsellsTotal + afterHoursSurcharge + weekendSurcharge

  // Discount logic — gift voucher takes priority and covers full price
  const hasAnyVoucher = !!(appliedVoucher || appliedGiftVoucher)
  const repeatCustomerDiscount = isRepeatCustomer && !hasAnyVoucher
    ? Math.round(subtotal * REPEAT_CUSTOMER_DISCOUNT_PERCENT) : 0

  const calculateVoucherDiscount = (voucher: Voucher, amount: number): number => {
    let discount = voucher.discount_type === 'fixed'
      ? voucher.discount_value
      : Math.round((amount * voucher.discount_value) / 100)
  ...
  const handleApplyVoucher = async () => {
    ...
    // 1. Check gift vouchers first (GV- prefix or via API)
    try {
      const res = await fetch(
        `/api/gift-vouchers/validate?code=${encodeURIComponent(upperCode)}&serviceSlug=${encodeURIComponent(service.slug)}&peopleCount=${formData.peopleCount}`
      )
      const data = await res.json()

      if (data.valid && data.giftVoucher) {
        setAppliedGiftVoucher(data.giftVoucher)
      ...
      // If gift voucher table returned an error that's specific (not just "not found"), show it
      if (!data.valid && data.error && upperCode.startsWith('GV-')) {
        setVoucherError(data.error)
        setVoucherLoading(false)
        return
      }
    } catch {
      // Network error checking gift vouchers — fall through to promo check
    }

    // 2. Check regular promo vouchers
    ...
    if (error || !voucher) { setVoucherError('Invalid voucher code'); setVoucherLoading(false); return }
    if (voucher.expires_at && new Date(voucher.expires_at) < new Date()) { setVoucherError('This voucher has expired'); setVoucherLoading(false); return }
    if (voucher.usage_limit && voucher.usage_count >= voucher.usage_limit) { setVoucherError('This voucher has reached its usage limit'); setVoucherLoading(false); return }
    if (voucher.min_spend > subtotal) { setVoucherError(`Minimum spend of R${voucher.min_spend} required`); setVoucherLoading(false); return }
  ...
  const giftVoucherDiscount = appliedGiftVoucher ? subtotal : 0  // covers full amount
  const activeDiscount = appliedGiftVoucher
    ? giftVoucherDiscount
    : appliedVoucher
    ? cappedVoucherDiscount
    : repeatCustomerDiscount
  const activeDiscountType: 'voucher' | 'repeat_customer' | 'gift_voucher' | null = appliedGiftVoucher
    ? 'gift_voucher'
    : appliedVoucher
    ? 'voucher'
    : isRepeatCustomer
  ...
  const pricing: BookingPricing = savedBooking
    ...
    : {
        ...
        discountType: activeDiscountType as any,
        finalTotal,
        depositAmount,
      }
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      const payload: CreateBookingPayload = {
        ...
        customerPregnancyWeeks: formData.clientPregnancyWeeks ?? null,
        ...
        // Gift voucher: pass code only (no voucherId — server looks up gift_vouchers table)
        voucherCode: appliedGiftVoucher
          ? appliedGiftVoucher.code
          : appliedVoucher?.code || null,
        voucherId: appliedGiftVoucher
          ? null
          : appliedVoucher?.id || null,
        voucherDiscount: appliedGiftVoucher
          ? giftVoucherDiscount
          : cappedVoucherDiscount,
        isZeroPayment,
    ...
    <div className="space-y-6">
      ...
      <div className="border border-gray-200 rounded-lg divide-y divide-gray-200">
        ...
        <div className="p-4 bg-gray-50">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-gray-700">{formData.peopleCount} {formData.peopleCount === 1 ? 'person' : 'people'}</p>
            </div>
            ...
            <div className="space-y-4">
              {perPersonSummaries.map((summary) => (
                <div key={summary.person} className="border-l-2 border-gray-200 pl-3">
                  <p className="text-sm font-medium text-gray-600 mb-2">Person {summary.person}</p>
          ...
          <div className="space-y-1 text-sm">
            <p className="text-gray-800">
              {formData.selectedDate
                ? new Date(formData.selectedDate).toLocaleDateString('en-ZA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
                : 'Not selected'}
              {formData.selectedTime && ` at ${formData.selectedTime}`}
              {isAfterHours && <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">After-hours</span>}
            </p>
            {formData.selectedTime && (
              <p className="text-gray-600">Duration: {totalDuration} minutes{totalDuration > service.duration_minutes && <span className="text-gray-500"> (includes add-ons)</span>}</p>
            )}
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="space-y-1 text-sm">
            {formData.clientAllergies && <p className="text-gray-800"><span className="text-gray-600 font-medium">Allergies:</span> {formData.clientAllergies}</p>}
            {formData.clientMedicalHistory && <p className="text-gray-800"><span className="text-gray-600 font-medium">Medical History:</span> {formData.clientMedicalHistory}</p>}
            {!formData.clientAllergies && !formData.clientMedicalHistory && <p className="text-gray-600 italic">No allergies or medical conditions noted</p>}
          </div>
        </div>

        {/* Voucher input — hidden once booking is saved */}
        {!savedBooking && (
          <div className="p-4">
            <h4 className="font-semibold text-gray-900 mb-3">Voucher or Gift Voucher Code</h4>

            {/* Applied gift voucher */}
            {appliedGiftVoucher && (
              <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                <div>
                  <p className="font-medium text-green-800">{appliedGiftVoucher.code}</p>
                  <p className="text-sm text-green-700">
                    🎁 Gift voucher — covers full amount (R{subtotal.toLocaleString()})
                  </p>
                  <p className="text-xs text-green-600 mt-0.5">
                    Valid for: {appliedGiftVoucher.serviceName} · {appliedGiftVoucher.peopleCount} {appliedGiftVoucher.peopleCount === 1 ? 'person' : 'people'}
                  </p>
                </div>
                <button onClick={handleRemoveVoucher} className="text-sm text-green-700 hover:text-green-900 font-medium">Remove</button>
              </div>
            )}

            {/* Applied promo voucher */}
            {appliedVoucher && !appliedGiftVoucher && (
              <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                <div>
                  <p className="font-medium text-green-800">{appliedVoucher.code}</p>
                  <p className="text-sm text-green-700">
                    {appliedVoucher.discount_type === 'fixed' ? `R${appliedVoucher.discount_value} off` : `${appliedVoucher.discount_value}% off`}
                  </p>
                </div>
                <button onClick={handleRemoveVoucher} className="text-sm text-green-700 hover:text-green-900 font-medium">Remove</button>
              </div>
            ...
            {/* Input — shown only when nothing is applied */}
            {!appliedVoucher && !appliedGiftVoucher && (
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Enter voucher or gift voucher code"
          ...
          <div className="space-y-2">
            ...
            {pricing.afterHoursSurcharge > 0 && (
              <div className="flex justify-between text-sm text-amber-800 bg-amber-50 -mx-4 px-4 py-2">
                <span>After-hours surcharge (R{AFTER_HOURS_SURCHARGE_PP} x {formData.peopleCount})</span>
                <span className="font-medium">R{pricing.afterHoursSurcharge}</span>
              </div>
            )}
            {pricing.weekendSurcharge > 0 && (
              <div className="flex justify-between text-sm text-amber-800 bg-amber-50 -mx-4 px-4 py-2">
                <span>Weekend / Public Holiday Surcharge (R{weekendSurchargePP} x {formData.peopleCount})</span>
            ...
            {pricing.discountAmount > 0 && pricing.discountType === 'voucher' && (
              <div className="flex justify-between text-sm text-green-800 bg-green-50 -mx-4 px-4 py-2">
                <span className="flex items-center gap-2">
                  Voucher Discount
                  {appliedVoucher && <span className="font-mono text-xs">({appliedVoucher.code})</span>}
                </span>
                <span className="font-medium">-R{pricing.discountAmount}</span>
              </div>
            )}
            {pricing.discountAmount > 0 && pricing.discountType === 'gift_voucher' && (
              <div className="flex justify-between text-sm text-green-800 bg-green-50 -mx-4 px-4 py-2">
                <span className="flex items-center gap-2">
                  🎁 Gift Voucher
                  {appliedGiftVoucher && <span className="font-mono text-xs">({appliedGiftVoucher.code})</span>}
                </span>
                <span className="font-medium">-R{pricing.discountAmount}</span>
              </div>
            )}
            ...
            {pricing.depositAmount > 0 ? (
              <div className="pt-2 border-t border-green-300 bg-green-50 -mx-4 px-4 py-2 mt-2">
                ...
                <p className="text-xs text-green-700 mt-1">Booking held for 20 minutes. Expires if payment not completed.</p>
              </div>
            ) : (
              <div className="flex justify-between text-green-800 font-semibold pt-2 border-t border-green-300 bg-green-50 -mx-4 px-4 py-2 mt-2">
                <span>{appliedGiftVoucher ? 'Fully Covered by Gift Voucher' : 'Fully Covered by Voucher'}</span>
                <span>R0</span>
              </div>
            )}
          </div>
        ...
        <div className="border border-green-300 bg-green-50 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-green-800">Status: {savedBooking.status === 'confirmed' ? 'Confirmed' : 'Awaiting Payment'}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!savedBooking && (
        <div className="flex items-start gap-3 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <label htmlFor="terms-checkbox" className="text-sm text-gray-700 cursor-pointer">
            I accept the{' '}
            <a href="https://crownedstudio.co.za/terms" target="_blank" rel="noopener noreferrer" className="text-gray-900 underline font-medium hover:text-gray-700" onClick={(e) => e.stopPropagation()}>
              Terms & Conditions
            </a>
          </label>
        </div>
      ...
      {!savedBooking ? (
        ...
        >
          {isCreatingBooking ? 'Creating Booking...' : isZeroPayment ? 'Confirm Booking' : 'Continue to Payment'}
        </button>
      ) : savedBooking.status === 'confirmed' ? (
        <a href="/booking/success" className="w-full bg-green-700 text-white py-3 px-6 rounded-lg font-semibold hover:bg-green-800 transition-colors text-center block">
          View Confirmation
        </a>
      ) : (
        <button
      ...
      <p className="text-sm text-gray-700 text-center">
        {!savedBooking
          ? isZeroPayment ? 'Your voucher fully covers this booking. Click to confirm.' : 'Your booking will be created and you will be redirected to payment'
